...
export type ModelType<T> = {
  [K in keyof ExcludeFunction<T>]?: T[K] extends Primitive
    ? T[K]
    : ModelType<T[K]>
}
export type ModelSnakeType<T> = {
  [K in keyof ExcludeFunction<T> as SnakeCase<K>]?: T[K] extends Primitive
    ? T[K]
    : ModelSnakeType<T[K]>
}

export function createModel<T>(
  model: new (dto: any, options?: IModelOptions) =>
    | T
    | (() => new (dto: any, options?: IModelOptions) => T),
